...
def format_prompt(samples):
    # Format the prompt, using prompt from the GPT3 paper
    # (https://arxiv.org/abs/2005.14165, Figure G.28)
    prompt_template = (
        "Tittel: {title}\n\nTekst: {text}\n\nSpørsmål: {question}\n\nSvar:{answer}"
    )
    examples = [
        prompt_template.format(
            ...
            if i < len(samples) - 1
            else "",  # Add space before target text
            # (except for the last example, which should be empty)
        )
...
def main():
    ...
    log_file.write(
        f"Mean EM: {mean(em_scores)} ± {stdev(em_scores) if len(em_scores) > 1 else 0}\n"
    )
    log_file.close()
