...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", type=str, default=None)  # Path to the pre-trained model
    parser.add_argument("--source_language", type=str, default="en")  # Source language code
    parser.add_argument("--target_language", type=str, default="nb")  # Target language code
    parser.add_argument("--n_shots", type=int, default=5)  # Number of random examples to sample
...
def format_prompt(source_texts: List[str], target_texts: List[List[str]], source_language: str, target_language: str):
    # Format the prompt with source and target texts
    source_language = {"en": "Engelsk", "nb": "Bokmål", "nn": "Nynorsk"}[source_language]
    target_language = {"en": "Engelsk", "nb": "Bokmål", "nn": "Nynorsk"}[target_language]

    # As in "The unreasonable effectiveness of few-shot learning for machine translation" (https://arxiv.org/abs/2302.01398)
    prompt_template = "{source_language}: {source_text}\n{target_language}:{target_text}"
    ...
    prompt = '\n\n'.join(examples)  # Join the examples with two newlines (https://arxiv.org/abs/2302.01398)
...
def load_data(source_language: str, target_language: str, reverse: bool = False):
    ...
    try:
        dataset = load_dataset("Helsinki-NLP/tatoeba_mt", f"{source_language_id}-{target_language_id}", split="test")
    except ValueError:
    ...
    for sample in dataset:
        source_text = ' '.join(sample["sourceString" if not reverse else "targetString"].strip().split())  # Remove leading/trailing whitespace and normalize whitespace
        target_text = ' '.join(sample["targetString" if not reverse else "sourceString"].strip().split())  # Remove leading/trailing whitespace and normalize whitespace
    ...
    deduplicated_dataset = sorted(deduplicated_dataset.items(), key=lambda x: (x[0], x[1]))
    deduplicated_dataset = [{"source_text": source_text, "target_texts": target_texts} for source_text, target_texts in deduplicated_dataset]
...
def main():
    ...
    log_file = open(f"eval_{args.source_language}_{args.target_language}_{args.model_name_or_path.split('/')[-1]}_{args.n_shots}-shots.txt", "w")
    ...
    for _ in range(args.n_repetitions):
        ...
        bert_score = torchmetrics.text.BERTScore("bert-base-multilingual-cased", num_layers=9)  # set according to https://docs.google.com/spreadsheets/d/1RKOVpselB98Nnh_EOC4A2BYn8_201tmPODpNWu4w7xI/edit#gid=0

        for i, example in enumerate(tqdm(dataset)):
            shots = sample_random_examples(dataset, i, args.n_shots)
            source_texts = [example["source_text"] for example in shots] + [example["source_text"]]
            target_texts = [example["target_texts"] for example in shots] + [example["target_texts"]]

            prompt = format_prompt(source_texts, target_texts, args.source_language, args.target_language)
    ...
    log_file.write(f"CHRF score: {mean(chrf_scores)} ± {stdev(chrf_scores) if len(chrf_scores) > 1 else 0}\n")
    log_file.write(f"BERT score: {mean(bert_scores)} ± {stdev(bert_scores) if len(bert_scores) > 1 else 0}")
