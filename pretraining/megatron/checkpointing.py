    ...
    if not torch.distributed.is_initialized() or mpu.get_data_parallel_rank() == 0 \
        ...
        if not args.deepspeed:
            ...
            try:
                ...
                print_rank_0(f"Error saving checkpoint to {checkpoint_name}")
...
def load_checkpoint(model, optimizer, lr_scheduler, load_arg='load', strict=True):
    ...
    if args.deepspeed:
        ...
        loaded_dir, state_dict = model[0].load_checkpoint(load_dir, load_optimizer_states=load_optimizer_states, load_lr_scheduler_states=load_optimizer_states, load_module_only=not load_optimizer_states)
    ...
    # Set iteration.
    if args.finetune or release or args.no_load_optim:
        iteration = 0
    else:
        try:
    ...
    if 'args' in state_dict and not args.no_load_optim:
    ...
    # rng states.
    if not release and not args.finetune and not args.no_load_rng and not args.no_load_optim:
        try:
