...
class GPTDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if doc_index_f == doc_index_l:
            ...
            attention_mask = torch.ones(len(sample), len(sample))
            position_ids = torch.arange(len(sample), dtype=torch.long)
    
        else:
            # Otherwise, get the rest of the initial document.
            sample_list = [self.indexed_dataset.get(self.doc_idx[doc_index_f], offset=offset_f)]
            attention_mask = torch.ones(len(sample_list[0]), len(sample_list[0]))
            position_ids = torch.arange(len(sample_list[0]), dtype=torch.long)

            # Loop over all in between documents and add the entire document.
            for i in range(doc_index_f + 1, doc_index_l):
                sample_list.append(self.indexed_dataset.get(self.doc_idx[i]))
                attention_mask = torch.block_diag(attention_mask, torch.ones(len(sample_list[-1]), len(sample_list[-1])))
                position_ids = torch.cat((position_ids, torch.arange(len(sample_list[-1]), dtype=torch.long)))
        ...
        # sample = torch.tensor(sample.astype(np.int64), dtype=torch.long)

        attention_mask = attention_mask[:-1, :-1]
        attention_mask = attention_mask.long()

        # tokens = sample[:-1].contiguous()
        # labels = sample[1:].contiguous()

        # # do masked language modeling with probability 10%
        # if "valid" not in self.name and "test" not in self.name and torch.rand(1).item() < 0.1:
        #     mask_index = 4

        #     # mask 16.67% of non-special tokens
        #     length = sample.size(0)
        #     span_lengths = torch.randint(1, 3 + 1, size=(length,), dtype=torch.int)
        ...
        #     mask_ratios = torch.rand([max_index + 1])[indices]
        ...
        # loss_mask = (labels >= 16).long()

        return {
            'tokens': np.array(sample, dtype=np.int64),
