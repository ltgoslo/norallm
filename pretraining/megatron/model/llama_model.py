...
from .transformer_llama import ParallelTransformerLayerPipe
...
class LlamaEmbedding(MegatronModule):
    ...
    def __init__(self,
        ...
        # self.init_method = init_method_normal_l2(0.01)

        # Word embeddings (parallel).
        self.word_embeddings = mpu.VocabParallelEmbedding(vocab_size, self.hidden_size,
                                                          init_method=self.init_method)
        #self.rms_norm = RMSNorm(self.hidden_size, eps=args.layernorm_epsilon, init_method=init_method_full(1.0))
...
class LlamaLMHead(MegatronModule):
    ...
    def __init__(self,
        ...
        if args.embedding_checkpoint is not None:
            rank = mpu.get_tensor_model_parallel_rank()
            path = f"{args.embedding_checkpoint}/layer_37-model_0{rank}-model_states.pt"
...
class LlamaModel(MegatronModule):
    ...
    def __init__(
        ...
        self.language_model, self._language_model_key = get_language_model(
            ...
            scaled_init_method=init_method_normal(0.01),
...
class LlamaModelPipe(PipelineModule,MegatronModule):
    ...
    def __init__(
        ...
        if args.hf_checkpoint is not None:
            hf_checkpoint = {
                **torch.load(f"{args.hf_checkpoint}/pytorch_model-00001-of-00002.bin"),
                **torch.load(f"{args.hf_checkpoint}/pytorch_model-00002-of-00002.bin"),
            }
        else:
            hf_checkpoint = None
        ...
        for layer_idx in range(args.num_layers):
            self.specs.append(
                LayerSpec(ParallelTransformerLayerPipe,
                    ...
                    output_layer_init_method=scaled_init_method_normal(0.01, args.num_layers),
        ...
        if hf_checkpoint is not None:
            last_norm_checkpoint = hf_checkpoint["model.norm.weight"]
        else:
