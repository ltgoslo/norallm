...
def init_method_normal_l2(l2_norm: float):
    def init_(tensor):
        new_tensor = torch.randn_like(tensor)
        new_tensor = F.normalize(new_tensor, dim=-1, p=2) * l2_norm
...
def init_method_full(value: float):
    def init_(tensor):
