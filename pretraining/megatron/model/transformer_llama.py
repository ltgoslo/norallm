...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method, layer_number=0, hf_checkpoint=None):
        ...
        if hf_checkpoint is not None:
            tensor_up = hf_checkpoint[f"model.layers.{layer_number}.mlp.up_proj.weight"].chunk(n_tp, 0)[tp_rank].to(self.dense_h_to_4h.weight)
            tensor_gate = hf_checkpoint[f"model.layers.{layer_number}.mlp.gate_proj.weight"].chunk(n_tp, 0)[tp_rank].to(self.dense_h_to_4h.weight)
        ...
        if hf_checkpoint is not None:
            tensor = hf_checkpoint[f"model.layers.{layer_number}.mlp.down_proj.weight"].chunk(n_tp, 1)[tp_rank].to(self.dense_4h_to_h.weight)
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, init_method,
        ...
        if hf_checkpoint is not None:
            tensor_k = hf_checkpoint[f"model.layers.{layer_number}.self_attn.k_proj.weight"].chunk(n_tp, 0)[tp_rank].to(self.query_key_value.weight)
            tensor_q = hf_checkpoint[f"model.layers.{layer_number}.self_attn.q_proj.weight"].chunk(n_tp, 0)[tp_rank].to(self.query_key_value.weight)
            tensor_v = hf_checkpoint[f"model.layers.{layer_number}.self_attn.v_proj.weight"].chunk(n_tp, 0)[tp_rank].to(self.query_key_value.weight)
            tensor = torch.cat([tensor_q, tensor_k, tensor_v], dim=0)
            assert tensor.shape == self.query_key_value.weight.shape, f"{tensor.shape} != {self.query_key_value.weight.shape}"
            self.query_key_value.weight.data = tensor
        ...
        if hf_checkpoint is not None:
            tensor = hf_checkpoint[f"model.layers.{layer_number}.self_attn.o_proj.weight"].chunk(n_tp, 1)[tp_rank].to(self.dense.weight)
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self, init_method, output_layer_init_method,
        ...
        if hf_checkpoint is not None:
            tensor = hf_checkpoint[f"model.layers.{layer_number}.input_layernorm.weight"].to(self.input_layernorm.weight)
        ...
        if hf_checkpoint is not None:
            tensor = hf_checkpoint[f"model.layers.{layer_number}.post_attention_layernorm.weight"].to(self.post_attention_layernorm.weight)
