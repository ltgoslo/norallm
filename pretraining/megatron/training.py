...
def get_learning_rate_scheduler(optimizer):
    ...
    lr_scheduler = AnnealingLR(
        ...
        cooldown_steps=args.lr_cooldown_samples,
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if writer and (iteration % args.tensorboard_log_interval == 0) and is_last_rank():
        ...
        try:
            ...
            wandb.log({
                "stats/consumed-train-samples": args.consumed_train_samples,
                "stats/consumed-train-tokens": args.consumed_train_tokens,
            }, step=iteration + args.wandb_step_offset, commit=False)
            ...
            if args.log_learning_rate_to_tensorboard:
                ...
                wandb.log({
                    "stats/learning-rate": learning_rate,
                }, step=iteration + args.wandb_step_offset, commit=False)
            ...
            if num_zeros_in_grad is not None:
                ...
                wandb.log({
                    "stats/num-zeros-in-grad": num_zeros_in_grad,
                }, step=iteration + args.wandb_step_offset, commit=False)
            ...
            if args.curriculum_learning:
                ...
                wandb.log({
                    "stats/curriculum-seqlen": args.curriculum_seqlen,
                }, step=iteration + args.wandb_step_offset, commit=False)
        ...
        if writer and is_last_rank():
            if args.log_timers_to_tensorboard:
                try:
                    ...
                    wandb.log({
                        "stats/iteration-time": elapsed_time_per_iteration,
                        "stats/samples-per-second": samples_per_sec,
                        "stats/samples-per-second-per-replica": samples_per_sec_per_replica,
                        "stats/tokens-per-second": tokens_per_sec,
                        "stats/tokens-per-second-per-replica": tokens_per_sec_per_replica,
                        "stats/TFLOPs-per-gpu-estimated": tflops,
                    }, step=iteration + args.wandb_step_offset, commit=True)
...
def train(forward_step_func, model, optimizer, lr_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        if args.save and args.save_interval and (iteration % args.save_interval == 0 or iteration <= 1):
    ...
    for key in total_loss_dict:
        ...
        if writer and is_last_rank():
            try:
                ...
                wandb.log({
                    f"validation/{key}": total_loss_dict[key].item(),
                })
            ...
            # wandb.log({}, step=iteration, commit=True)
